...
const chart = document.querySelector('#chart') // Get out graph element (`const`, does not change)
...
const parseData = (data) => {
	...
	chart.style.setProperty('--gray', grayCount)
	chart.style.setProperty('--cinnamon', cinnamonCount)
	chart.style.setProperty('--black', blackCount)
	chart.style.setProperty('--undefined', undefinedCount)
}
...
caches.open('cachedData') // Set up a cache for our data
    ...
    for (var i = 0; i < chartjson.data.length; i++) {
        ...
        var prefix = chartjson.prefix || '';bardata.appendChild(bar);
